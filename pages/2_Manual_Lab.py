...
st.set_page_config(page_title="ATP Manual Match Lab",page_icon="🧪",layout="wide")
st.title("🧪 Manual Match Lab")
st.caption("Optional manual sandbox. The default app page is now the automated ATP 250+ live board.")
...
# Compute freshness at page load instead of trusting a stale flag written by the
# last successful workflow. If GitHub Actions stops, the warning still ages.
latest_model_date = pd.to_datetime(metrics.get("latest_data_date"), errors="coerce")
if pd.notna(latest_model_date):
    match_age = max(0, int((pd.Timestamp.now(tz="UTC").tz_localize(None).normalize() - latest_model_date.normalize()).days))
else:
    match_age = freshness.get("matches",{}).get("age_days")
if match_age is not None and match_age > 7:
    st.error(
...
with st.sidebar:
    ...
    level_label=st.selectbox(
        "Tournament level",
        ["ATP 250","ATP 500","Masters 1000","ATP Finals","Grand Slam"],
        index=0,
    )
    level_map={
        "ATP 250":2.0,
    ...
    if override_speed:
        court_speed=st.slider(
            "Court speed (0.55 very slow – 1.55 very fast)",
            min_value=0.55,max_value=1.55,value=float(round(estimated_speed,2)),step=0.01,
        )
    else:
        court_speed=None
    ...
    best_of=5 if level_label=="Grand Slam" else 3
    st.caption(f"Format is automatic: best-of-{best_of} for {level_label}.")
...
with st.expander("Head-to-head model inputs", expanded=True):
    if h2h_matches == 0:
        st.info("No recorded prior head-to-head matches were found. H2H features are neutral.")
    else:
        ...
        h2.metric(
            ...
            help=f"{h2h_surface_matches} prior meetings on {surface}",
        )
        h3.metric("Shrunk overall edge", f"{float(r.get('h2h_overall_edge', 0.0)):+.3f}")
        h4.metric("Shrunk surface edge", f"{float(r.get('h2h_surface_edge', 0.0)):+.3f}")
        st.caption(
            "H2H contributes from the first prior meeting, but small samples are strongly shrunk toward neutral. "
            "A 3–0 record therefore matters modestly rather than being ignored or treated as a 100% signal. "
            "There is no manual probability bump after the model prediction."
        )

if st.button("Save prediction / bet to tracking"):
...
def metric_table(row):
    metrics = [
        ...
        ("Serve rating (EWMA)", row.serve),
        ("Return rating (EWMA)", row.return_rating),
        ("Last 3 win rate", row.win3),
        ("Last 5 win rate", row.win5),
        ("Last 10 win rate", row.win10),
        ("Surface last 10", row.surface_win10),
        ("Service points won — last match", row.spw1),
        ("Service points won — last 5", row.spw5),
        ("Return points won — last match", row.rpw1),
        ("Return points won — last 5", row.rpw5),
        ("1st serve in — last 5", row.first_in5),
        ("1st serve points won — last 5", row.first_won5),
        ("2nd serve points won — last 5", row.second_won5),
        ("Ace rate — last 5", row.ace_rate5),
        ("Double-fault rate — last 5", row.df_rate5),
        ("Point share — last 5", row.point_share5),
        ("Break points saved — last 5", row.bp_save5),
        ("Break points converted — last 5", row.bp_convert5),
        ("Recent form EWMA", row.form_ewma),
        ("Surface form EWMA", row.surface_form_ewma),
        ...
        ("Winner rate — advanced coverage", row.winner_rate),
        ("UE rate — advanced coverage", row.ue_rate),
        ("Advanced-stat coverage", row.advanced_coverage),
        ("Average 1st-serve speed", row.avg_first_serve_speed),
    ...
    for name, value in metrics:
        if pd.isna(value):
            ...
            if any(token in name.lower() for token in percentage_names):
                value = f"{value:.1%}"
...
st.caption(
    f"Prediction context: {level_label} · court speed {r['court_speed']:.2f} "
    f"(prior {float(r.get('court_speed_prior') or r['court_speed']):.2f}, "
    f"live weight {float(r.get('court_speed_live_weight', 0.0)):.0%}) · "
