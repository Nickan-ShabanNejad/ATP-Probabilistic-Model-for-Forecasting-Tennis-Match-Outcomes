...
with b2:
    st.caption(
        "Set this once to show current bankroll, peak bankroll, and maximum drawdown. "
        "The live board also multiplies quarter-Kelly by the current bankroll to recommend a CA$ stake."
    )
...
if open_rows:
    labels = {
        x.id: f"#{x.id} — {x.selection or x.player_a} · {x.market_type} ({x.tournament or x.surface})"
        for x in open_rows
    }
    ...
    result = st.selectbox(f"Did {selected.selection or selected.player_a} win?", ["Yes", "No"])
    auto_a = auto_b = 0.0
    if selected.event_id:
        snap = last_pre_start_snapshot(selected.event_id, selected.start_timestamp)
        if snap and selected.market_type == "Moneyline":
            if str(snap.get("player_a")) == str(selected.player_a):
                auto_a, auto_b = float(snap.get("odds_a") or 0), float(snap.get("odds_b") or 0)
            elif str(snap.get("player_b")) == str(selected.player_a):
                auto_a, auto_b = float(snap.get("odds_b") or 0), float(snap.get("odds_a") or 0)
        elif snap and selected.market_type == "Total Sets 3.5":
            over = float(snap.get("sets_over35") or 0)
            under = float(snap.get("sets_under35") or 0)
            if str(selected.selection).casefold().startswith("over"):
                auto_a, auto_b = over, under
            else:
                auto_a, auto_b = under, over
        if auto_a and auto_b:
            st.success(f"Stored last pre-start Pinnacle snapshot found: {auto_a:.3f} / {auto_b:.3f}. Leave the fields unchanged to use it as the close.")
    c1, c2 = st.columns(2)
    with c1:
        closing_a = st.number_input("Closing decimal odds selection", min_value=0.0, value=float(auto_a), step=0.01)
    with c2:
        closing_b = st.number_input("Closing decimal odds opposite side", min_value=0.0, value=float(auto_b), step=0.01)
    st.caption("For moneylines and Total Sets 3.5, the app uses the last stored Pinnacle quote strictly before scheduled start when available. Enter a close manually only if no usable snapshot was captured.")
