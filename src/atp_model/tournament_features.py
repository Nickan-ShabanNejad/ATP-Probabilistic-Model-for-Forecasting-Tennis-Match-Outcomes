...
def canonical_tournament(value: str) -> str:
    ...
    # Provider names vary heavily and often include sponsor + city names. Collapse
    # the major recurring ATP venues to stable keys before any lookup. This avoids
    # exact-event history/court-speed misses such as ``U.S. Open - New York`` vs
    # ``US Open`` or ``Cincinnati Open - Cincinnati`` vs ``Cincinnati``.
    compact = re.sub(r"^u\s+s(?:\s+|$)", "us ", compact).strip()
    if (compact.startswith("us ") or compact == "us") and "new york" in (compact + " new york"):
        return "us new york"
    if "australian" in compact:
        return "australian melbourne"
    if "roland garros" in compact or compact.startswith("french"):
        return "french paris"
    if "wimbledon" in compact:
        return "wimbledon"
    if "indian wells" in compact:
        return "indian wells"
    if "miami" in compact:
        return "miami"
    if "monte carlo" in compact:
        return "monte carlo"
    if "madrid" in compact:
        return "madrid"
    if "rome" in compact or "italia" in compact:
        return "rome"
    if "national bank" in compact or "canadian" in compact or compact.startswith("canada"):
        return "canadian"
    if "cincinnati" in compact:
        return "cincinnati"
    if "shanghai" in compact:
        return "shanghai"
    if "paris" in compact and "french" not in compact:
        return "paris"
    if "finals" in compact or "turin" in compact:
        return "finals"
...
def load_surface_speeds(path: Path) -> pd.DataFrame:
    ...
    try:
        ...
        return pd.DataFrame()
    # Always rebuild the canonical key from the tournament name when available.
    # Older generated CSVs carried stale keys such as ``us open`` while the live
    # provider resolves to ``us new york``; trusting that stale column made exact
    # lookups silently fall back to a neutral hard-court median (1.00).
    if "tournament" in frame.columns:
        frame["tournament_key"] = frame["tournament"].map(canonical_tournament)
    elif "tournament_key" in frame.columns:
        frame["tournament_key"] = frame["tournament_key"].map(canonical_tournament)
...
def court_speed_label(speed: float) -> str:
    """Human-readable label for the model's 0.55-1.55 court-speed scale."""
    value = float(speed)
    if value < 0.78:
        return "slow"
    if value < 0.93:
