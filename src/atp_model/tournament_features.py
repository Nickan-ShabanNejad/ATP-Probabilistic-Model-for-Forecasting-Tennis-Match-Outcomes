...
def canonical_tournament(value: str) -> str:
    ...
    # Provider names vary heavily (for example "US Open", "U.S. Open - New York",
    # and "US Open - New York"). Collapse the major Slam naming variants so
    # exact-event history and live-slate tournament context resolve to one key.
    compact = re.sub(r"^u\s+s(?:\s+|$)", "us ", compact).strip()
    if compact in {"us", "us new york"}:
        return "us new york"
    if compact in {"australian", "australian melbourne"}:
        return "australian melbourne"
    if compact in {"roland garros", "french", "french paris"}:
        return "french paris"
    if compact in {"wimbledon", "wimbledon london"}:
        return "wimbledon"
