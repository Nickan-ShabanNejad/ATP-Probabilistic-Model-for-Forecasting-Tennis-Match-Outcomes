...
def _h2h_model_state(player_a, player_b, surface):
    ...
    raw = (a_wins - b_wins) / matches if matches else 0.0
    sraw = (sa - sb) / surface_matches if surface_matches else 0.0
    overall_edge = raw * matches / (matches + 4.0) if matches else 0.0
    surface_edge = sraw * surface_matches / (surface_matches + 3.0) if surface_matches else 0.0
    ...
    def metric_diff(row, p1_col, p2_col, evidence):
        ...
        return float(raw_diff * evidence / (evidence + 3.0)) if evidence else 0.0
    ...
    record = {
        ...
        "used_as_predictive_edge": matches > 0,
    }
    return record
