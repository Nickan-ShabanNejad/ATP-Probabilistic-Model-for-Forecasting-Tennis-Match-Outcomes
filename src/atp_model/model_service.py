...
def _market_metrics(probability, odds_self, odds_other):
    fair = 1 / float(probability)
    try:
        oa, ob = float(odds_self), float(odds_other)
    except Exception:
        return {"market": np.nan, "edge": np.nan, "ev": np.nan, "fair": fair, "quarter_kelly": 0.0}
    if not np.isfinite(oa) or not np.isfinite(ob) or oa <= 1 or ob <= 1:
        return {"market": np.nan, "edge": np.nan, "ev": np.nan, "fair": fair, "quarter_kelly": 0.0}
    ia, ib = 1/oa, 1/ob
    market = ia/(ia+ib)
    ev = probability*oa-1
    full_kelly = max(0.0, ev/(oa-1.0))
    return {"market":market,"edge":probability-market,"ev":ev,"fair":fair,"quarter_kelly":full_kelly*.25}
...
def predict_match(df,bundle,player_a,player_b,surface,rank_a,rank_b,odds_a,odds_b,tournament_level=2.0,best_of=None,tournament="",prediction_year=None,court_speed_override=None,indoor=False,api_best_of=None):
    ...
    a_ev_ok = np.isfinite(ma["ev"])
    b_ev_ok = np.isfinite(mb["ev"])
    if a_ev_ok and ma["ev"] > (mb["ev"] if b_ev_ok else -np.inf) and ma["ev"] > 0:
        side, pick, pick_ev, pick_edge, pick_kelly, pick_odds = "A", player_a, ma["ev"], ma["edge"], ma["quarter_kelly"], float(odds_a)
    elif b_ev_ok and mb["ev"] > 0:
        side, pick, pick_ev, pick_edge, pick_kelly, pick_odds = "B", player_b, mb["ev"], mb["edge"], mb["quarter_kelly"], float(odds_b)
    else:
        evs = [x for x in (ma["ev"], mb["ev"]) if np.isfinite(x)]
        edges = [x for x in (ma["edge"], mb["edge"]) if np.isfinite(x)]
        side, pick = "NO BET", "No bet"
        pick_ev = max(evs) if evs else np.nan
        pick_edge = max(edges) if edges else np.nan
        pick_kelly, pick_odds = 0.0, np.nan
