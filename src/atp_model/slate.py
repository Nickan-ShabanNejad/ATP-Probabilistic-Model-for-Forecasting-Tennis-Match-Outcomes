...
def _start_timestamp(event: dict) -> float:
    """Return a provider event start time as Unix seconds.

    Matchstat surfaces have returned both second and millisecond Unix timestamps.
    Normalizing in one place prevents every event from being rejected as far-future
    when a 13-digit timestamp appears.
    """
    raw = event.get("startTimestamp") or event.get("timestamp") or event.get("startTime") or 0
    try:
...
def _is_grand_slam(ctx: dict, best_of: float | None = None) -> bool:
    level = float(ctx.get("level", 0) or 0)
    bo = float(best_of if best_of is not None else (ctx.get("best_of", 0) or 0))
    return abs(level - 5.0) < 1e-9 and bo >= 5.0
...
def eligible_events(
    ...
    *,
    ...
    target_date = selected_date if selected_date is not None else (local_today if today_only else None)
    kept: list[dict] = []
    diag = {
        "lower_tier": 0, "unknown_context": 0, "qualifying": 0,
        "bad_status": 0, "bad_time": 0, "wrong_date": 0, "not_today": 0,
    }
    for event in events:
        ...
        if status not in {"not started", "upcoming", "scheduled", ""}:
            diag["bad_status"] += 1
            continue
        start = _start_timestamp(event)
        if start and start < now - 300:
            diag["bad_time"] += 1
            continue
        # A chosen calendar date is authoritative.  The old rolling horizon is kept
        # only for callers that are not filtering to a specific day.
        if target_date is None and start and start > max_ts:
            diag["bad_time"] += 1
            continue
        if target_date is not None and start:
            event_date = datetime.fromtimestamp(start, tz=timezone.utc).astimezone(tz).date()
            if event_date != target_date:
                diag["wrong_date"] += 1
                if selected_date is None and today_only:
                    diag["not_today"] += 1
                continue
...
def _event_odds(client: MatchstatClient, event: dict, pinnacle_client: PinnOddsClient | None = None, force: bool = False) -> dict:
    ...
    wants_sets35 = _is_grand_slam(ctx)
    ...
    def fetch_for_id(target_id: str) -> dict:
        ...
        try:
            ...
            sets35 = pinnacle_total_sets_35_from_payload(payload) if wants_sets35 else None
            if ml is not None:
                source = "pre-match"
        except Exception as exc:
            error = str(exc)
        ...
        if ml is None:
            try:
                ...
                if wants_sets35 and sets35 is None:
    ...
    for event in events:
        ...
        if not a or not b:
            ...
            rows.append({
                ...
                "Tournament": str(ctx.get("tournament") or league), "Level": ctx["level"], "Surface": surface, "BO": int(ctx["best_of"]),
        ...
        gs_sets = None
        if _is_grand_slam(ctx, result.get("best_of")):
        ...
        row = {
            ...
            "Match": f"{a} vs {b}",
            "Tournament": str(ctx.get("tournament") or league),
            "Level": ctx["level"],
            "Surface": surface,
