...
from .odds import pinnacle_moneyline_from_payload, pinnacle_total_sets_35_from_payload
...
def _event_odds(client: MatchstatClient, event: dict, force: bool = False) -> dict:
    ...
    error = None
    try:
        # One request can contain all available pre-match markets and Pinnacle.
        payload = client.pre_match_odds(eid)
        ml = pinnacle_moneyline_from_payload(payload)
        sets35 = pinnacle_total_sets_35_from_payload(payload)
    except Exception as exc:
        error = str(exc)

    # Compare-market is a moneyline fallback only, so normal refreshes stay at one
    # API request per event whenever pre-match odds contains the needed quote.
    if ml is None:
        try:
            compared = client.compared_odds(eid, market_id=1)
            ml = pinnacle_moneyline_from_payload(compared)
        except Exception as exc:
            error = error or str(exc)

    out = {"fetched_at": now, "moneyline": ml, "sets35": sets35, "error": error, "payload": payload}
    _ODDS_CACHE[eid] = out
    return out
...
def build_slate(
    ...
    *,
    ...
    for event in events:
        ...
        league = str(event.get("league") or "").strip()
        ctx = event["_ctx"]
        if not a or not b or a not in player_names or b not in player_names:
            continue
        surface = ctx["surface"]
        ...
        rec_ok = (
            result["recommended_side"] != "NO BET"
        ...
        model_pick_p = (
            result["probability_a"] if result["recommended_side"] == "A"
            else result["probability_b"] if result["recommended_side"] == "B"
            else np.nan
        )
        row = {
            "Event ID": eid,
            ...
            "ML Edge": result["recommended_edge"] if rec_ok else max(result["edge_a"], result["edge_b"]),
            "ML EV": result["recommended_ev"],
            "ML Kelly %": ml_kelly,
