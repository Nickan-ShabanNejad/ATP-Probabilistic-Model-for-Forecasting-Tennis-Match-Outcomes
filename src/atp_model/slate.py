...
def _event_odds(client: MatchstatClient, event: dict, force: bool = False) -> dict:
    ...
    books: set[str] = set()
    try:
        # First preference: current pre-match feed. It can contain all markets.
    ...
    # Compare-market is the most explicit moneyline endpoint.
    if ml is None:
        try:
    ...
    # Final pre-start fallback: Matchstat's recent-odds feed. Eligible events are
    # not-started only, so this cannot accidentally substitute in-play prices.
    if ml is None:
        try:
            recent = client.recent_odds(eid)
    ...
    out = {
        "fetched_at": now, "moneyline": ml, "sets35": sets35, "error": error,
        "source": source, "available_bookmakers": sorted(books, key=str.casefold),
