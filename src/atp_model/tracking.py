...
def _normalize_record(raw: dict[str, Any], fallback_id: int) -> dict[str, Any]:
    ...
    normalized = {
        ...
        "start_timestamp": _float_or_none(raw.get("start_timestamp")),
        "market_type": str(raw.get("market_type") or "Moneyline"),
        "selection": str(raw.get("selection") or raw.get("player_a") or ""),
...
def save_prediction(result: dict[str, Any], odds_a: Any, odds_b: Any, stake: Any = 0.0, notes: str = "") -> int:
    ...
    record = {
        ...
        "start_timestamp": _float_or_none(result.get("start_timestamp")),
        "market_type": str(result.get("market_type") or "Moneyline"),
        "selection": str(result.get("selection") or result.get("player_a") or ""),
...
def current_bankroll() -> float | None:
    ...
    if start is None:
        return None
    profit = sum(float(r.profit or 0.0) for r in get_predictions() if r.result_a is not None)
