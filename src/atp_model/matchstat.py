...
def _payload_rows(payload: Any) -> list[dict]:
    """Extract a list of records from the response shapes used across Matchstat v2.

    Matchstat has more than one upcoming/events surface and the wrappers are not
    completely uniform (e.g. data/results/events/matches).  Keep this tolerant so a
    harmless response-wrapper change cannot make the live board suddenly go empty.
    """
    ...
    for key in ("data", "singles", "results", "events", "matches", "items", "result"):
...
def _normalize_upcoming_event(row: dict) -> dict:
    """Normalize Core/Extend upcoming records to the live-event schema used by the app."""
    if not isinstance(row, dict):
        return {}
    out = dict(row)

    p1 = _name_from(
        row.get("participant1") or row.get("player1") or row.get("homePlayer")
        or row.get("home") or row.get("competitor1") or row.get("team1")
    )
    p2 = _name_from(
        row.get("participant2") or row.get("player2") or row.get("awayPlayer")
        or row.get("away") or row.get("competitor2") or row.get("team2")
    )
    if p1:
        out["participant1"] = p1
    if p2:
    ...
    tournament = row.get("tournament") or row.get("league") or row.get("competition")
    league = _name_from(tournament) if tournament is not None else ""
    if league:
        out["league"] = league

    if not out.get("id"):
        out["id"] = row.get("eventId") or row.get("matchId") or row.get("gameId") or row.get("fixtureId")

    if not out.get("startTimestamp"):
        ts = row.get("timestamp") or row.get("startTime") or row.get("start_timestamp")
        if isinstance(ts, (int, float)):
            out["startTimestamp"] = ts
        elif isinstance(ts, str) and ts:
            try:
                parsed = pd.to_datetime(ts, utc=True, errors="raise")
                out["startTimestamp"] = float(parsed.timestamp())
            except Exception:
                pass

    if not out.get("status"):
        status = row.get("matchStatus") or row.get("state")
        if isinstance(status, dict):
            status = status.get("name") or status.get("type")
        out["status"] = status or "Upcoming"

    out.setdefault("tourType", row.get("tourType") or row.get("tour") or "ATP")
    if not out.get("name") and p1 and p2:
        out["name"] = f"{p1} vs {p2}"
    return out
...
class MatchstatClient:
    ...
    def upcoming_events(self, tour: str = "atp", max_events: int = 80) -> list[dict]:
        """Return upcoming events with redundant Matchstat fallbacks.

        The Extend upcoming feed is preferred because its event IDs map directly to
        the odds endpoints.  If Matchstat transiently returns an empty wrapper, fall
        back to the documented Core upcoming surfaces instead of blanking the board.
        The cached client resource also keeps the last non-empty slate for 30 minutes;
        downstream time/status filters still remove stale matches automatically.
        """
        rows: list[dict] = []
        self.last_upcoming_source = "extend"
        primary_error = None
        try:
            page = 1
            while len(rows) < max_events:
                ...
                if not batch:
                    break
                rows.extend(_normalize_upcoming_event(x) for x in batch)
        ...
        # Fallback 1: today's important ATP matches. This is particularly useful
        # late in a Slam/Masters day when there are only a handful of matches left.
        if not rows:
            try:
                payload = self.get(f"/tennis/v2/upcoming/top-tennis-matches-today/{tour}")
                batch = _payload_rows(payload)
                rows = [_normalize_upcoming_event(x) for x in batch]
                if rows:
                    self.last_upcoming_source = "top-today"
            except Exception:
                pass

        # Fallback 2: general ATP singles upcoming feed, paginated.
        if not rows:
            try:
                page = 1
                while len(rows) < max_events:
                    payload = self.get(
                        f"/tennis/v2/upcoming/matches/{tour}",
                        params={"group": "singles", "page": page, "limit": min(50, max_events - len(rows))},
                    )
                    batch = _payload_rows(payload)
                    if not batch:
                        break
                    rows.extend(_normalize_upcoming_event(x) for x in batch)
                    if len(batch) < min(50, max_events - len(rows) + len(batch)):
                        break
                    page += 1
                    if page > 10:
                        break
                if rows:
                    self.last_upcoming_source = "core-upcoming"
            except Exception:
                pass

        # Avoid a single transient empty API response erasing a working live board.
        now = time.time()
        rows = [x for x in rows if isinstance(x, dict) and (x.get("participant1") or x.get("name"))]
        if rows:
            self._last_good_upcoming = (now, rows[:max_events])
            return rows[:max_events]

        cached = getattr(self, "_last_good_upcoming", None)
        if cached and now - float(cached[0]) <= 1800:
            self.last_upcoming_source = "last-good-cache"
            return list(cached[1])[:max_events]

        if primary_error is not None:
            raise RuntimeError(f"Upcoming Matchstat feeds returned no usable events; primary error: {primary_error}")
        return []
    ...
    def all_markets(self) -> Any:
        return self.get("/tennis/v2/extend/api/markets/all")

    def pre_match_odds(self, event_id: int | str, market_ids: str | None = None) -> Any:
        params = {"market_ids": str(market_ids)} if market_ids else None
        return self.get(
            f"/tennis/v2/extend/api/odds/pre-match/{event_id}",
            params=params,
        )
