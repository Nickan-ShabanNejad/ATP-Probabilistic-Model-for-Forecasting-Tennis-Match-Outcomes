...
def charting_profile(events, player_name, date):
    # Charting events are sorted by date. Use binary search instead of scanning
    # a player's full history on every training row; the old scan became
    # increasingly expensive in recent seasons.
    key = normalize_name(player_name)
    rows = events.get(key, [])
    if not rows:
        return {"chart_winner_rate": np.nan, "chart_ue_rate": np.nan, "chart_available": 0.0}
    dates = chart_event_dates.get(key)
    if dates is None:
        dates = [x["date"] for x in rows]
    idx = bisect_left(dates, date)
    recent = rows[max(0, idx - 20):idx]
    if not recent:
        return {"chart_winner_rate": np.nan, "chart_ue_rate": np.nan, "chart_available": 0.0}
...
chart_event_dates = {player: [row["date"] for row in rows] for player, rows in chart_events.items()}
...
# v0.3: player-specific performance relative to pre-match Elo expectation.
# Exact-event and event-level histories are shrunk toward zero; speed history is
# stored as a leakage-safe ridge regression of excess performance on court speed.
event_perf_history = defaultdict(lambda: {"sum": 0.0, "n": 0})
level_perf_history = defaultdict(lambda: {"sum": 0.0, "n": 0})
speed_perf_history = defaultdict(lambda: {"xy": 0.0, "xx": 0.0, "n": 0})
EVENT_PERF_PRIOR = 8.0
LEVEL_PERF_PRIOR = 20.0
SPEED_RIDGE = 0.35
SPEED_MATCH_PRIOR = 12.0

# Separate Grand-Slam match-length model state.
sets_rows = []
gs_sets_history = defaultdict(lambda: {"n": 0, "over": 0, "five": 0, "straight": 0})
event_sets_history = defaultdict(lambda: {"n": 0, "over": 0, "five": 0, "straight": 0})
SETS_PRIOR = 10.0
...
def _shrunk_perf(record, prior):
    n = int(record.get("n", 0))
    return float(record.get("sum", 0.0)) / (n + float(prior)) if n > 0 else 0.0


def _speed_slope(record):
    n = int(record.get("n", 0))
    if n <= 0:
        return 0.0
    raw = float(record.get("xy", 0.0)) / (float(record.get("xx", 0.0)) + SPEED_RIDGE)
    return float(raw * n / (n + SPEED_MATCH_PRIOR))


def context_state(pid_a, pid_b, tournament, level, speed):
    event = canonical_tournament(str(tournament or ""))
    level_key = float(level)
    ea = event_perf_history[(str(pid_a), event)]
    eb = event_perf_history[(str(pid_b), event)]
    la = level_perf_history[(str(pid_a), level_key)]
    lb = level_perf_history[(str(pid_b), level_key)]
    sa = speed_perf_history[str(pid_a)]
    sb = speed_perf_history[str(pid_b)]
    slope_a, slope_b = _speed_slope(sa), _speed_slope(sb)
    return {
        "event_perf_edge": _shrunk_perf(ea, EVENT_PERF_PRIOR) - _shrunk_perf(eb, EVENT_PERF_PRIOR),
        "level_perf_edge": _shrunk_perf(la, LEVEL_PERF_PRIOR) - _shrunk_perf(lb, LEVEL_PERF_PRIOR),
        "speed_fit_edge": slope_a - slope_b,
        "speed_fit_current_interaction": (float(speed) - 1.0) * (slope_a - slope_b),
        "event_matches_log": float(math.log1p(min(int(ea["n"]), int(eb["n"])))),
        "level_matches_log": float(math.log1p(min(int(la["n"]), int(lb["n"])))),
        "speed_fit_matches_log": float(math.log1p(min(int(sa["n"]), int(sb["n"])))),
    }


def _score_set_count(score):
    text = str(score or "").upper()
    if any(marker in text for marker in RETIREMENT_MARKERS):
        return None
    # Each completed set begins with a games score such as 6-4 or 7-6(5).
    return len(re.findall(r"(?<!\d)\d{1,2}\s*-\s*\d{1,2}", text)) or None


def _sets_profile(record):
    n = int(record.get("n", 0))
    denom = n + SETS_PRIOR
    return {
        "over": (float(record.get("over", 0)) + 0.5 * SETS_PRIOR) / denom,
        "five": (float(record.get("five", 0)) + 0.2 * SETS_PRIOR) / denom,
        "straight": (float(record.get("straight", 0)) + 0.45 * SETS_PRIOR) / denom,
        "n": n,
    }


def sets_feature_values(pid_a, pid_b, a, b, tournament, surface, speed, level, elo_expected_a):
    event = canonical_tournament(str(tournament or ""))
    ga, gb = _sets_profile(gs_sets_history[str(pid_a)]), _sets_profile(gs_sets_history[str(pid_b)])
    ea, eb = _sets_profile(event_sets_history[(str(pid_a), event)]), _sets_profile(event_sets_history[(str(pid_b), event)])
    surface_code = {"Hard": 0.0, "Clay": 1.0, "Grass": 2.0}.get(str(surface).title(), 0.0)
    return [
        1.0 - abs(2.0 * float(elo_expected_a) - 1.0),
        abs(float(a["surface_elo"]) - float(b["surface_elo"])) / 400.0,
        ...
        float(speed),
        (float(speed) - 1.0) * ((float(a["serve"]) + float(b["serve"])) / 2.0),
        (ga["over"] + gb["over"]) / 2.0,
        abs(ga["over"] - gb["over"]),
        (ea["over"] + eb["over"]) / 2.0,
        (ga["five"] + gb["five"]) / 2.0,
        (ga["straight"] + gb["straight"]) / 2.0,
        math.log1p(min(ga["n"], gb["n"])),
        math.log1p(min(ea["n"], eb["n"])),
        surface_code,
        float(level),
    ]
...
def feature_values(a, b, rank_a, rank_b, age_a, age_b, level, best_of, speed, speed_meta, indoor, h2h, context):
    def diff(key):
    ...
    values = {
        ...
        "event_perf_edge": context["event_perf_edge"],
        "level_perf_edge": context["level_perf_edge"],
        "speed_fit_edge": context["speed_fit_edge"],
        ...
        "speed_fit_current_interaction": context["speed_fit_current_interaction"],
        ...
        "event_matches_log": context["event_matches_log"],
        "level_matches_log": context["level_matches_log"],
        "speed_fit_matches_log": context["speed_fit_matches_log"],
    }
...
# ---------------------------------------------------------------------------
for row_number, r in enumerate(matches.to_dict("records"), start=1):
    if row_number % 10000 == 0:
        print(f"feature pass: {row_number:,}/{len(matches):,}", flush=True)
    date = r["date"]
    ...
    context_features = context_state(winner, loser, r.get("tourney_name", ""), level, court_speed)
    features = feature_values(
        winner_state, loser_state, wrank, lrank, wage, lage, level, best_of,
        court_speed, speed_meta, indoor, h2h, context_features,
    )

    # Grand-Slam O/U 3.5 sets gets its own independent probability model.
    set_count = _score_set_count(r.get("score", ""))
    if level >= 5.0 and float(best_of) >= 5.0 and set_count is not None and set_count >= 3:
        pre_ws = float(surface_elo[(winner, surface)])
        pre_ls = float(surface_elo[(loser, surface)])
        set_expected = 1 / (1 + 10 ** ((pre_ls - pre_ws) / 400))
        sets_rows.append({
            "date": date,
            "year": date.year,
            "y": int(set_count >= 4),
            "x": sets_feature_values(
                winner, loser, winner_state, loser_state, r.get("tourney_name", ""),
                surface, court_speed, level, set_expected,
            ),
        })
    flip = rng.random() < 0.5
    ...
    # Contextual event/level/speed residuals are measured against the player's
    # pre-match SURFACE Elo expectation. This avoids calling ordinary surface
    # suitability (for example, a clay specialist on clay) a court-speed effect.
    w_perf = 1.0 - expected_surface
    l_perf = -w_perf

    event_name = canonical_tournament(str(r.get("tourney_name", "")))
    for pid, perf in ((winner, w_perf), (loser, l_perf)):
        er = event_perf_history[(str(pid), event_name)]
        er["sum"] += float(perf); er["n"] += 1
        lr = level_perf_history[(str(pid), float(level))]
        lr["sum"] += float(perf); lr["n"] += 1
        sr = speed_perf_history[str(pid)]
        sx = float(court_speed) - 1.0
        sr["xy"] += sx * float(perf); sr["xx"] += sx * sx; sr["n"] += 1

    if level >= 5.0 and float(best_of) >= 5.0:
        set_count = _score_set_count(r.get("score", ""))
        if set_count is not None and set_count >= 3:
            over = int(set_count >= 4); five = int(set_count >= 5); straight = int(set_count == 3)
            for pid in (winner, loser):
                for key in (str(pid),):
                    gr = gs_sets_history[key]
                    gr["n"] += 1; gr["over"] += over; gr["five"] += five; gr["straight"] += straight
                evr = event_sets_history[(str(pid), event_name)]
                evr["n"] += 1; evr["over"] += over; evr["five"] += five; evr["straight"] += straight
...
metrics = {
    ...
    "feature_groups": {
        ...
        "event_history": [x for x in FEATURES if x.startswith("event_")],
        "level_history": [x for x in FEATURES if x.startswith("level_perf")],
    },
    "charting": chart_meta,
    "court_speed_rows": int(len(empirical_speeds)),
    "court_speed_source": prior_speed_source,
    "pipeline_version": "5.0-v03-event-level-speedfit-automated-slate",
}
...
# Context-profile snapshots used by the live prediction service.
event_profile_rows = []
for (pid, event), record in event_perf_history.items():
    if int(record["n"]) <= 0:
        continue
    event_profile_rows.append({
        "player": names.get(str(pid), str(pid)), "tournament": event,
        "event_perf": _shrunk_perf(record, EVENT_PERF_PRIOR), "matches": int(record["n"]),
    })
pd.DataFrame(event_profile_rows).to_csv(GENERATED / "player_event_profiles.csv.gz", index=False, compression="gzip")

level_profile_rows = []
for (pid, level_key), record in level_perf_history.items():
    if int(record["n"]) <= 0:
        continue
    level_profile_rows.append({
        "player": names.get(str(pid), str(pid)), "level": float(level_key),
        "level_perf": _shrunk_perf(record, LEVEL_PERF_PRIOR), "matches": int(record["n"]),
    })
pd.DataFrame(level_profile_rows).to_csv(GENERATED / "player_level_profiles.csv.gz", index=False, compression="gzip")

speed_profile_rows = []
for pid, record in speed_perf_history.items():
    if int(record["n"]) <= 0:
        continue
    speed_profile_rows.append({
        "player": names.get(str(pid), str(pid)), "speed_slope": _speed_slope(record), "matches": int(record["n"]),
    })
pd.DataFrame(speed_profile_rows).to_csv(GENERATED / "player_speed_profiles.csv.gz", index=False, compression="gzip")

sets_profile_rows = []
for pid, record in gs_sets_history.items():
    p = _sets_profile(record)
    sets_profile_rows.append({
        "player": names.get(str(pid), str(pid)), "scope": "Grand Slam", "tournament": "",
        "over35_rate": p["over"], "five_set_rate": p["five"], "straight_rate": p["straight"], "matches": p["n"],
    })
for (pid, event), record in event_sets_history.items():
    p = _sets_profile(record)
    sets_profile_rows.append({
        "player": names.get(str(pid), str(pid)), "scope": "Event", "tournament": event,
        "over35_rate": p["over"], "five_set_rate": p["five"], "straight_rate": p["straight"], "matches": p["n"],
    })
pd.DataFrame(sets_profile_rows).to_csv(GENERATED / "player_sets_profiles.csv.gz", index=False, compression="gzip")

SETS_FEATURES = [
    "elo_closeness", "surface_elo_gap", "avg_serve", "serve_gap", "avg_return", "avg_ace_rate",
    "point_share_gap", "court_speed", "speed_avg_serve", "avg_gs_over35", "gs_over35_gap",
    "avg_event_over35", "avg_five_set_rate", "avg_straight_rate", "gs_matches_log",
    "event_matches_log", "surface_code", "tournament_level",
]
sets_metrics = {"available": False, "training_matches": 0}
if len(sets_rows) >= 500:
    s_latest_year = max(row["year"] for row in sets_rows)
    s_holdout_year = s_latest_year - 1 if s_latest_year >= 2025 else s_latest_year
    s_train = [row for row in sets_rows if row["year"] < s_holdout_year]
    s_test = [row for row in sets_rows if row["year"] == s_holdout_year]
    if len(s_train) < 300 or len(s_test) < 50:
        split = int(len(sets_rows) * 0.85); s_train, s_test = sets_rows[:split], sets_rows[split:]; s_holdout_label = "latest chronological 15%"
    else:
        s_holdout_label = str(s_holdout_year)
    sx_train = np.array([r["x"] for r in s_train], dtype=float); sy_train = np.array([r["y"] for r in s_train], dtype=int)
    sx_test = np.array([r["x"] for r in s_test], dtype=float); sy_test = np.array([r["y"] for r in s_test], dtype=int)
    sets_candidates = [
        ("logistic_regression", Pipeline([("impute", SimpleImputer(strategy="median")), ("scale", StandardScaler()), ("model", LogisticRegression(C=0.7, max_iter=5000))])),
        ("hist_gradient_boosting", Pipeline([("impute", SimpleImputer(strategy="median")), ("model", HistGradientBoostingClassifier(learning_rate=0.04, max_iter=250, max_leaf_nodes=12, min_samples_leaf=35, l2_regularization=4.0, early_stopping=True, random_state=321))])),
    ]
    sets_evals = []
    for sname, sest in sets_candidates:
        sest.fit(sx_train, sy_train); sp = sest.predict_proba(sx_test)[:, 1]
        sets_evals.append({"name": sname, "pipeline": sest, "log_loss": float(log_loss(sy_test, sp)), "brier": float(brier_score_loss(sy_test, sp)), "accuracy": float(accuracy_score(sy_test, sp >= 0.5))})
    sw = min(sets_evals, key=lambda x: (x["log_loss"], x["brier"]))
    stemplate = dict(sets_candidates)[sw["name"]]
    stemplate.fit(np.array([r["x"] for r in sets_rows], dtype=float), np.array([r["y"] for r in sets_rows], dtype=int))
    sets_metrics = {"available": True, "training_matches": len(s_train), "test_matches": len(s_test), "holdout": s_holdout_label, "selected_model": sw["name"], "log_loss": sw["log_loss"], "brier": sw["brier"], "accuracy": sw["accuracy"], "features": SETS_FEATURES}
    joblib.dump({"pipeline": stemplate, "features": SETS_FEATURES, "metrics": sets_metrics}, MODEL / "sets_model.joblib")
metrics["sets_model"] = sets_metrics
