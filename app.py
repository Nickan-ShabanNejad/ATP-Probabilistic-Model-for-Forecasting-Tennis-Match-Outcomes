...
st.caption(
    "Automated ATP 250+ slate · Pinnacle prices · model probabilities · EV · quarter-Kelly · "
    "bankroll stakes · click any match for the full breakdown."
)
...
def live_board():
    ...
    st.caption(
        f"Board refreshed {now} · {len(eligible)} eligible ATP 250+ events inspected · "
        f"{len(board)} currently have usable Pinnacle moneyline prices."
    )
    if board.empty:
        st.info("No eligible ATP 250+ matches currently have a complete model match + Pinnacle moneyline price.")
        with st.expander("Diagnostics"):
            st.json(diag)
        return

    recommended = board[board["Best market"] != "No bet"]
    ...
    display_cols = [
        "Start", "Tournament", "Level", "Match", "BO",
