...
st.caption(
    f"Latest included match date: {metrics.get('latest_data_date','unknown')} · "
    f"2025 chronological holdout accuracy: {metrics.get('accuracy_2025',0):.1%} · "
    f"log loss: {metrics.get('log_loss_2025',0):.3f}"
)
...
r=predict_match(df,model,pa,pb,surface,rank_a,rank_b,odds_a,odds_b)
...
def metric_table(row):
    return pd.DataFrame({
      ...
      "Rest days","Recent Elo change","Last match"],
      ...
      row.rest_days,row.elo_change10,row.last_match]
    })
